export function deterministicReply(text, now = new Date()) {
  ...
  if (/^(고마워|고마워요|고맙습니다|감사합니다|감사해|thanks|thankyou)$/u.test(exact)) return '도움이 되어 기뻐요. 더 필요한 것이 있으면 말씀해 주세요.';
  if (/^(알겠어|알겠어요|좋아|좋아요|오케이|ok|okay)$/u.test(exact)) return '네, 좋아요. 필요하신 게 있으면 말씀해 주세요.';
  if (/^(도움말|도와줘|뭘할수있어|무엇을할수있어|사용법|help|whatcanyoudo)$/u.test(exact)) return '질문 답변과 쇼핑·예약·이동 같은 요청 정리를 도와드릴 수 있어요. 필요한 일을 한 문장으로 말씀해 주세요.';
