...
assert.match(conversation, /sendButton\.disabled\s*=\s*inFlight\s*\|\|\s*attachmentUploading\s*\|\|\s*\(prompt\.value\.trim\(\)\.length\s*===\s*0\s*&&\s*selectedAttachments\.length\s*===\s*0\)/);
...
assert.match(conversation, /if \(inFlight \|\| attachmentUploading\) return/);
