...
const calendarEntryVersion = '20260924-placecompact1';
const calendarManagerVersion = '20260924-placecompact1';
const calendarModelVersion = '20260923-daysheet3';
const calendarCssVersion = '20260924-placecompact1';
...
assert.ok(css.includes('.calendar-editor-backdrop {\n  position: fixed;'), 'event editor must cover the viewport instead of anchoring to an unpositioned content box');
assert.ok(css.includes('.calendar-editor-dialog {\n  box-sizing: border-box;'), 'event editor shell contract missing');
assert.ok(css.includes('.calendar-editor-form { flex: 1 1 auto; min-height: 0;'), 'event editor form must allow the inner body to shrink and scroll');
assert.ok(css.includes('.calendar-editor-body {'), 'event editor needs a dedicated scrolling body');
assert.ok(css.includes('-webkit-overflow-scrolling: touch;'), 'iOS touch scrolling contract missing');
assert.ok(css.includes('env(safe-area-inset-top)'), 'iPhone top safe area contract missing');
assert.ok(css.includes('env(safe-area-inset-bottom)'), 'iPhone bottom safe area contract missing');
assert.ok(css.includes('height: var(--calendar-editor-visual-height, 100dvh);'), 'mobile editor must follow the visual viewport');
