...
function wrapperMarkup(w,h){
  ...
  return '<!doctype html><html><body style="margin:0">'+['feb','sep','may'].map(frame).join('')+'<pre id="result">pending</pre><script>'+
    ...
    'setTimeout(()=>{if(out.textContent==="pending"){out.textContent=JSON.stringify({ok:false,error:"wrapper timeout"});clearInterval(timer)}},75000);'+
    '</script></body></html>';
}
...
function run(browser,w,h){
  ...
  const proc=spawnSync(browser,[
    ...
    '--window-size=1600,1000','--force-device-scale-factor=1','--virtual-time-budget=80000',
