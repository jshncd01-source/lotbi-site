...
const FIXTURE_REL = 'scripts/.profile-menu-runtime-fixture.html';
...
const rect=document.querySelector('.profile-popover').getBoundingClientRect(),mobile=innerWidth<=900;if(rect.left<-1||rect.right>innerWidth+1||rect.top<-1||rect.bottom>innerHeight+1)throw new Error('popover viewport');
...
function run(browser,w,h){const r=spawnSync(browser,['--headless=new','--no-sandbox','--disable-gpu','--disable-dev-shm-usage','--window-size='+w+','+h,'--force-device-scale-factor=1','--virtual-time-budget=3500','--dump-dom',ORIGIN+'/'+FIXTURE_REL],{encoding:'utf8',timeout:30000,maxBuffer:12*1024*1024});if(r.error)throw r.error;if(r.status!==0)throw new Error('browser '+r.status+' '+r.stderr);const a='<pre id="profile-result">',b='</pre>',i=r.stdout.indexOf(a),j=r.stdout.indexOf(b,i);if(i<0||j<0)throw new Error('result missing');const raw=r.stdout.slice(i+a.length,j).replaceAll('&quot;','"').replaceAll('&amp;','&').replaceAll('&lt;','<').replaceAll('&gt;','>');const v=JSON.parse(raw);if(!v.ok)throw new Error(v.error);return v}

const browser=browserPath();fs.writeFileSync(FIXTURE,fixture,'utf8');const server=spawn('python',['-m','http.server',String(PORT),'--bind','127.0.0.1'],{cwd:ROOT,stdio:'ignore'});
try{waitServer();const desktop=run(browser,1440,900),mobile=run(browser,390,844);for(const [label,v] of [['desktop',desktop],['mobile',mobile]]){if(v.counts.me!==1||v.counts.subscription!==1||v.counts.logout!==1||v.counts.other!==0)throw new Error(label+' request contract');if(!v.continuity.urlUnchanged||v.continuity.draft!=='작성 중인 초안'||v.continuity.messageCount!==2)throw new Error(label+' continuity')}if(desktop.viewport.mobile||!mobile.viewport.mobile)throw new Error('desktop/mobile media query');console.log('SITE-PROFILE-MENU-IMPROVEMENT-01 desktop',JSON.stringify(desktop));console.log('SITE-PROFILE-MENU-IMPROVEMENT-01 mobile',JSON.stringify(mobile));console.log('SITE-PROFILE-MENU-IMPROVEMENT-01 RUNTIME PASS')}finally{server.kill('SIGTERM');fs.rmSync(FIXTURE,{force:true})}
