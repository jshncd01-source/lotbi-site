...
await import('/site-deterministic.js');
...
}catch(e){const errorDetails={};for(const key of Object.getOwnPropertyNames(e||{})){try{errorDetails[key]=String(e[key])}catch{}}out.textContent=JSON.stringify({ok:false,error:String((typeof importPhase==='string'?importPhase+': ':'')+(e?.stack||e)),errorDetails,counts})}
</script></body></html>`;
