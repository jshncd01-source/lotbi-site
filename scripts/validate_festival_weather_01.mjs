...
// date heading's aria-label so a screen reader never hears the icon twice.
assert.ok(festivalUiJs.includes("badge.setAttribute('aria-hidden', 'true')"));
assert.ok(festivalUiJs.includes("dateHeading.setAttribute('aria-label', buildProgramDateAriaLabel(group.date, weatherItem))"));
