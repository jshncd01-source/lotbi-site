...
try {
  ...
  for (const [w, h] of [[390, 844], [1280, 900]]) {
    ...
    // 직접 등록 became 자세히 when the title box moved into the panel: typing
    // the title IS the direct entry now, and this button is the longer form
    // behind it. The picture route keeps its name and its place in front of it.
    if (v.buttonOrder.join('|') !== '이미지로 등록|자세히') {
      throw new Error(`${label}: the day panel must offer 이미지로 등록 then 자세히, got ${v.buttonOrder.join('|')}`);
    }
